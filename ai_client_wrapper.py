...
class AI_Client_Call_Wrapper():
    ...
    # def prepare_image_for_open_ai(self, image_input: Union[str, Path, BytesIO], mime: str = 'image/png') -> str:
    #     """
    ...
    # def get_file_data(self, url: Union[str, Path]) -> Tuple[Union[BufferedReader, bytes], str]:
    def get_file_data(self, url: str | Path) -> tuple[BufferedReader|bytes, str]:
        """
    ...
    def call_image_text(self, ready_img_url:str, img_name:str=None, mime:str='image/png'):
        ...
        response = self.ai_client.chat.completions.create(model=self.configs['image_text']['model'],
        temperature=0,
        messages=[{"role": "user","content": [{"type": "text","text": "Stwórz opis obrazka, jakie widzisz tam elementy?"},
                {"type": "image_url","image_url": {
                        'url': ready_img_url,#"url": prepare_image_for_open_ai("dall_e_3__kot_siedzi_na_parapecie.png"),
                        "detail": "high"
                                                    },
                },
                                            ],}],)
